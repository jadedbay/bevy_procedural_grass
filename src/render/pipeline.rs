...
impl FromWorld for GrassGeneratePipeline {
   fn from_world(world: &mut World) -> Self {
        ...
        Self {
            chunk_layout,
            material_layout: world.resource::<MaterialPipeline<GrassMaterial>>().material_layout.clone(),
            clump_layout,
            shader: world.resource::<AssetServer>().load("embedded://bevy_procedural_grass/shaders/compute_grass.wgsl"),
        }
   } 
}
...
pub(crate) fn prepare_generate_pipeline(
    ...
    for entity in &query {
        ...
        commands.entity(entity).insert(GrassGeneratePipelineId(pipeline_id));
    }
}
